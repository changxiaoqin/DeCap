...
class FeatureDataset_with_syn(torch.utils.data.Dataset):
    def __init__(self, image_encoder, dataloader, syn_path, device):
        self.data = get_features(image_encoder, dataloader, device)
        fp = os.path.join(syn_path, "features.pt")
        lp = os.path.join(syn_path, "labels.pt")
        self.syn_data = {'features': torch.load(fp), "labels": torch.load(lp)}

    def __len__(self):
        return len(self.data['features']) + len(self.syn_data['features'])

    def __getitem__(self, idx):
        if idx < len(self.data['features']):
            data = {k: v[idx] for k, v in self.data.items()}
            data['features'] = torch.from_numpy(data['features']).float()
        else:
            idx = idx - len(self.data['features'])
            data = {k: v[idx] for k, v in self.syn_data.items()}
            data['features'] = torch.from_numpy(data['features']).float()
        return data['features'], data['labels']


def get_dataloader(dataloader, is_train, args, device, image_encoder=None, with_syn=False, syn_path=None):
    if image_encoder is not None:
        if with_syn:
            feature_dataset = FeatureDataset_with_syn(image_encoder, dataloader, syn_path, device)
    ...
    for epoch in range(epoches):
        ...
        for batch in learner_data:
            ...
            with torch.enable_grad():
                ...
                loss.backward()
            torch.nn.utils.clip_grad_norm_(learner.model.parameters(), 1.0)
        ...
        pbar.update(1)
    pbar.close()
    losses = min_loss / length
    accuracies = max_epoch_accuracy / length
